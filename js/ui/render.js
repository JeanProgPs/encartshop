...
const UIRender = (() => {
  ...
  function orderAdminCard(o, paymentSummary) {
    ...
    // Badge de pagamento online (só aparece quando há dados financeiros)
    const pmBadge = paymentBadge(paymentSummary);

    // Detalhe expandido do pagamento (só quando confirmado)
    const pmDetail = paymentDetail(paymentSummary);
  ...
  /**
   * Retorna o HTML do badge de status financeiro.
   * Sem dados → string vazia (sem alteração visual para pedidos WhatsApp).
   *
   * @param {object|null} summary - objeto de OrderPaymentAPI.getSummaryByOrders
   * @returns {string}
   */
  function paymentBadge(summary) {
    ...
    const configs = {
      // status em order_payments (lowercase — alinhado com Fase 6)
      confirmed: { emoji: '🟢', label: 'PIX Pago',      cls: 'bg-success/10 text-success border-success/20' },
      pending:   { emoji: '🟡', label: 'Aguardando PIX', cls: 'bg-warning/10 text-warning border-warning/20' },
      expired:   { emoji: '🔴', label: 'PIX Expirado',  cls: 'bg-danger/10 text-danger border-danger/20'   },
      cancelled: { emoji: '⚫', label: 'PIX Cancelado', cls: 'bg-textSecondary/10 text-textSecondary border-textSecondary/20' },
      refunded:  { emoji: '🟣', label: 'PIX Estornado', cls: 'bg-purple-500/10 text-purple-600 border-purple-200' },
    };

    const cfg = configs[summary.status] || { emoji: '💠', label: `PIX ${summary.status}`, cls: 'bg-info/10 text-info border-info/20' };

    return `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-bold ${cfg.cls}" title="Status do pagamento online" data-order-payment-badge>
      ${cfg.emoji} ${escapeHTML(cfg.label)}
    </span>`;
  }

  /**
   * Retorna o HTML do bloco de detalhe financeiro (exibido apenas quando pago).
   * Nunca expõe API Key, tokens ou informações sensíveis internas.
   *
   * @param {object|null} summary
   * @returns {string}
   */
  function paymentDetail(summary) {
    if (!summary) return '';
    // Exibe detalhes apenas quando confirmado (lowercase)
    if (summary.status !== 'confirmed') return '';

    const paidDate = summary.paid_at
      ? new Date(summary.paid_at).toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })
      : '—';
    const amount = fmtPrice(summary.amount);
    const gateway = escapeHTML((summary.gateway || 'asaas').charAt(0).toUpperCase() + (summary.gateway || 'asaas').slice(1));
    // Exibe apenas os últimos 8 chars do gateway_payment_id — nunca o ID interno completo
    const chargeRef = summary.gateway_payment_id
      ? '…' + escapeHTML(String(summary.gateway_payment_id).slice(-8))
      : '—';

    return `
      <div class="mt-3 mb-2 px-3 py-2.5 rounded-lg bg-success/5 border border-success/15 flex flex-wrap gap-x-6 gap-y-1">
        <div class="flex items-center gap-1.5 text-xs text-textSecondary">
          <span class="font-semibold text-textPrimary">Método:</span> ${escapeHTML(summary.billing_type || 'PIX')}
        </div>
        <div class="flex items-center gap-1.5 text-xs text-textSecondary">
          <span class="font-semibold text-textPrimary">Valor pago:</span> ${amount}
        </div>
        <div class="flex items-center gap-1.5 text-xs text-textSecondary">
          <span class="font-semibold text-textPrimary">Data:</span> ${paidDate}
        </div>
        <div class="flex items-center gap-1.5 text-xs text-textSecondary">
          <span class="font-semibold text-textPrimary">Gateway:</span> ${gateway}
        </div>
        <div class="flex items-center gap-1.5 text-xs text-textSecondary">
          <span class="font-semibold text-textPrimary">Ref.:</span> <span class="font-mono">${chargeRef}</span>
        </div>
      </div>`;
  }

  return { emptyState, fmtPrice, fmtDateShort, productAdminCard, productStoreCard, orderAdminCard, paymentBadge, paymentDetail };
})();
