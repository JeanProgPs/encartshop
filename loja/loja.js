/**
 * EncartShop — Loja Pública v15
 * Refatorado para trabalhar com StoreBootstrap.
 * Remove dependência de `DOMContentLoaded` interno e encapsula lógica.
 */
...
window.LojaApp = (() => {
  ...
  function renderCategoryTabs() {
    ...
    const tabs = [
      `<button class="cat-tab ${activeCategory==='Todos'?'active':''}" onclick="LojaApp.setCategory('Todos')">Todos</button>`
    ];
    if (hasPromo) {
      tabs.push(`<button class="cat-tab ${activeCategory==='Ofertas'?'active':''}" onclick="LojaApp.setCategory('Ofertas')" style="${activeCategory!=='Ofertas'?'color:#ef4444;border-color:#ef4444':''}">🔥 Ofertas</button>`);
    }
    categories.forEach(cat => {
      tabs.push(`<button class="cat-tab ${activeCategory===cat?'active':''}" onclick="LojaApp.setCategory(${JSON.stringify(cat)})">${cat}</button>`);
    });
      ...
      return `
        <div class="cart-item-row">
          ...
          <div class="cart-qty-control">
            <button class="cart-qty-btn remove" onclick="LojaApp.changeQty('${item.id}',-1)" title="Remover">−</button>
            <span class="cart-qty-num">${qtyLabel}</span>
            <button class="cart-qty-btn" onclick="LojaApp.changeQty('${item.id}',1)" title="Adicionar">+</button>
          </div>
        </div>`;
    }).join('');
...
// Expõe métodos para os eventos inline HTML e render.js
window.addToCart = window.LojaApp.addToCart;
window.changeQty = window.LojaApp.changeQty;
window.checkout = window.LojaApp.checkout;
window.setCategory = window.LojaApp.setCategory;
window.openCart = window.LojaApp.openCart;
window.closeCart = window.LojaApp.closeCart;
window.handleCartOverlayClick = window.LojaApp.handleCartOverlayClick;
